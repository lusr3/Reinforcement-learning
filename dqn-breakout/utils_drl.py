...
class Agent(object):
    ...
    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        idx_batch, state_batch, action_batch, reward_batch, next_batch, done_batch = \
            memory.sample(batch_size)

        values = self.__policy(state_batch.float()).gather(1, action_batch)
        action_max = self.__policy(next_batch.float()).argmax(1).unsqueeze(1)
        values_next = self.__target(next_batch.float()).gather(1, action_max).detach()
        expected = (self.__gamma * values_next) * \
            (1. - done_batch) + reward_batch                                    
        update_pri = abs(expected - values)
        ...
        memory.batch_update(idx_batch, update_pri)      
